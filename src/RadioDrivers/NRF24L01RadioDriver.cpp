 ...
 *  timeoutMilliseconds     Read time out in mS.
 * 
 */
bool NRF24L01RadioDriver::receive(char *buffer, uint8_t bufferSize, uint16_t timeoutMilliseconds)
{
    ...
    while (true)
    {
        ...
        if (packet[NRF24L01_PACKET_HEADER_DATA_LEN] > bufferSize)
        {
            this->stopReceiving();
            this->errOVFL++;
